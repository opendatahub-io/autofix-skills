...
**Scripts** handle all deterministic operations: JSON merging, state persistence, CVE routing decisions. Each skill ships its own scripts under `scripts/` within the skill directory. The LLM calls these via `python3 ${CLAUDE_SKILL_DIR}/scripts/<name>.py` and reads the output. `state.py` lives under `autofix-resolve/scripts/` and is symlinked from `autofix-cve-resolve/scripts/`.
