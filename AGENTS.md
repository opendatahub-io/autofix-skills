...
- **Context missing**: The outer layer (autofix repo) writes context files before the agent runs. For `autofix-resolve` and `autofix-cve-resolve`, context is in `.autofix-context/`. For `autofix-triage`, context is in `.triage-context/`. If the agent lacks info, the issue is likely in the outer layer's `context_writer`, not here. Check the appropriate context directory's `ticket.json` and `config.json`.
